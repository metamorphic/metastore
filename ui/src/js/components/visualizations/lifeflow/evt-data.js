...
var evtData = function () {
  ...
  /*
   origData:       array of raw event objects, each expected to have an
      entityId, eventName, and a start date string

   timelineArray:  origData grouped by entityId. each entity has a records
      array pointing to its raw event records and each of those should have
      a pointer back to the timeline and should be sorted in time order and
      should have nextEvt/prevEvt pointers as appropriate

   */
  function edata() {}

  function toDate(dateStr, fmt) { // storing dates as moment.js objects
  ...
  Evt.prototype.dtStr = function (unit) {
    ...
    switch (unit) {
      case 'year':
        fmt = 'YYYY';
        break;  // need a way to specify other fmts
      case 'month':
        fmt = 'MMM YYYY';
        break;
      case 'day':
      ...
      case 'hour':
        ...
        break;
      case 'minute':
        fmt = 'MM/DD/YYYY hh:mm:sa';
        break;
      default:
        fmt = 'MM/DD/YYYY hh:mm:SSSa';
        break;
    }
  ...
  Timelines.prototype.timelineUnit = function (arg) {
    ...
    if (typeof this._timelineUnit === "undefined" || arg) {
      this._timelineUnit = edata.durationUnits(this.maxDuration(null, arg));
    }
  ...
  Evt.prototype.unitSettings = function (opts) {
    return this.timeline().unitSettings(opts)
  };

  Timeline.prototype.unitSettings = function (opts) {
    return this.timelines().unitSettings(opts)
  };
  ...
  Evt.prototype.restoreUnitSettings = function () {
    return this.timeline().restoreUnitSettings()
  };

  Timeline.prototype.restoreUnitSettings = function () {
    return this.timelines().restoreUnitSettings()
  };
